...
class Run_model :
    ...
    def talib (self): # ตัวแปร
        ...
        if self.input_1 == 'jv':
            ...
            dataset['input_1'] = dataset.OHLC4.map(lambda x : nextprime(np.log(x) , self.length_1))
        else:
        ...
        if self.input_2 == 'jv':
            ...
            dataset['input_2'] = dataset.OHLC4.map(lambda x : nextprime(np.log(x) , self.length_2))
        else:
